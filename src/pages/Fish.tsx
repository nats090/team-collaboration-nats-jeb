...
export default function Fish() {
  ...
  const { data: productsData, isLoading } = useQuery({
    queryKey: ["fish-products", category?.id, currentPage],
    enabled: !!category,
    queryFn: async () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (product) => {
      ...
      logActivity({
        ...
        description: `Deleted fish product: ${product?.name}`,
      });
    },
  });
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-info">Fish Products</h1>
            <p className="text-muted-foreground">Manage your fish inventory</p>
          </div>
          {isAdmin && (
            <Button onClick={() => setIsAddDialogOpen(true)} className="bg-info hover:bg-info/90">
