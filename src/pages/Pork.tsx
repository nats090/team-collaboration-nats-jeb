...
export default function Pork() {
  ...
  const { data: productsData, isLoading } = useQuery({
    queryKey: ["pork-products", category?.id, currentPage],
    enabled: !!category,
    queryFn: async () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (product) => {
      ...
      logActivity({
        ...
        description: `Deleted pork product: ${product?.name}`,
      });
    },
  });
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-secondary">Pork Products</h1>
            <p className="text-muted-foreground">Manage your pork inventory</p>
          </div>
          {isAdmin && (
